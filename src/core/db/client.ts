...
let rawSqliteInstance: Database | null = null
...
export function initDb(customPath?: string) {
  ...
  const dbPath = customPath ?? join(config.DATA_DIR, 'aulaplay.db')
  ...
  // Configure SQLite WAL mode and safety settings
  if (dbPath !== ':memory:') {
...
export function getDb() {
  ...
  return dbInstance as NonNullable<typeof dbInstance>
}
...
export function getRawDb() {
  ...
  return rawSqliteInstance as NonNullable<typeof rawSqliteInstance>
}

export function closeDb() {
  if (rawSqliteInstance) {
    rawSqliteInstance.close()
    rawSqliteInstance = null
    dbInstance = null
  }
}

export function createTestDb() {
  const { db, sqlite } = initDb(':memory:')
  return { db, sqlite }
}
